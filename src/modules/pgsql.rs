...
/// PostgreSQL login module for performing PostgreSQL authentication
pub struct PgsqlModule {
    /// Target host
    host: String,
    /// Target port
    port: u16,
...
impl PgsqlModule {
    /// Create a new PostgreSQL module with default settings
    pub fn new() -> Self {
        Self {
            host: String::new(),
            port: 5432,
...
impl Module for PgsqlModule {
    ...
    fn initialize(&mut self) {
        ...
        if self.host.is_empty() {
            panic!("Host must be set for PostgreSQL module");
        }
    }
    ...
    fn attack(&self, payload: String) -> Result<Response, String> {
        // Format payload as username/password for PostgreSQL (simplified)
        ...
        match TcpStream::connect_timeout(&addr.parse().unwrap(), Duration::from_secs(self.timeout))
        {
            Ok(mut stream) => {
                ...
                // Simulate sending credentials (in real implementation, would handle PostgreSQL startup packet)
                ...
                let success = !buffer.is_empty(); // In reality, we would check for specific PostgreSQL success packets

                let status_code = if success { 200 } else { 401 };
                let body = format!(
                    "PostgreSQL response for {}: {}",
                    credentials,
                    if success {
                        "Login successful"
                    } else {
